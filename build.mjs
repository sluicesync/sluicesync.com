...
// The number of engines.Register(...) calls in the sluice repo (one per
// registered engine name). Used in the engines-page prose and enforced by the
// build-time drift check at the bottom of this file against a sibling
// C:\code\sluice checkout: if sluice registers a different number, the build
// fails naming both counts.
const ENGINE_COUNT = 13;
    ...
    items: [
      ...
      { slug: "how-sluice-copies", label: "How sluice copies your data" },
      {
        ...
        subs: [
          ...
          ["cutover", "cutover"],
          ["backup", "backup"],
          ["backup-export-parquet", "backup export-as-parquet"],
          ["restore", "restore"],
          ["backfill", "backfill"],
          ["expand-contract", "expand-contract"],
          ["deploy-ddl", "deploy-ddl"],
          ["control-tables", "control-tables ddl"],
...
live <strong>Cloudflare D1</strong> databases, <strong>CSV / TSV / NDJSON</strong> files, and <strong>mydumper / pscale dump</strong>
directories also import into Postgres or MySQL, and SQLite is itself a migrate target —
${ENGINE_COUNT} engines are registered today (run <code>sluice engines</code> to list them). It is built around three surfaces you can
...
${cmd(
  ...
  <tr><td><code>csv</code> / <code>tsv</code></td><td class="desc">migrate <strong>source</strong> (file)</td><td class="desc">RFC 4180 delimited files (ADR-0163), staged into a temp SQLite database with every value byte-exact TEXT (<code>007.1500</code> stays <code>007.1500</code>; integers above 2<sup>53</sup> stay exact) and read through the validated SQLite surface; <code>--infer-types</code> auto-engages to recover rich target types. File conventions are <strong>declared, never sniffed</strong> — see the flat-file note below. <code>tsv</code> is the same engine with the delimiter fixed to TAB. Migrate only (no CDC).</td></tr>
  <tr><td><code>ndjson</code></td><td class="desc">migrate <strong>source</strong> (file)</td><td class="desc">One JSON object per line (ADR-0163). Numbers land as their <strong>raw source text</strong> — never through a float64 — so integers above 2<sup>53</sup> and arbitrary-precision decimals stay byte-exact; nested objects/arrays land as raw JSON text; a duplicate key within an object or a top-level JSON array refuses loudly (the message carries the <code>jq -c '.[]'</code> conversion). Migrate only (no CDC).</td></tr>
  <tr><td><code>mydumper</code></td><td class="desc">migrate <strong>source</strong> (directory)</td><td class="desc">A mydumper or <code>pscale database dump</code> directory (ADR-0161): <code>metadata</code> + per-table <code>-schema.sql</code> + extended-INSERT data chunks (plain, gzip, or zstd). Values decode through the live MySQL engine's own decoder — byte-identical to a live read; single-precision FLOAT display-rounding baked into the dump file itself is WARNed per table. Migrate only (no CDC).</td></tr>
  </tbody></table>
  <div class="note"><strong>Flat-file imports: conventions are declared, never sniffed (ADR-0163).</strong> RFC 4180 encodes neither NULL nor header presence — both are producer conventions, and guessing either is the #1 CSV silent-loss class (a wrong header guess eats a data row or turns data into column names; a wrong NULL guess silently turns empty strings into NULLs or vice versa). So the <code>csv</code>/<code>tsv</code> drivers require the conventions on the command line:
  <table><thead><tr><th>Flag</th><th>Purpose</th></tr></thead><tbody>
  <tr><td><code>--csv-header</code> / <code>--csv-no-header</code></td><td class="desc"><strong>One is required.</strong> Declare whether the first record carries the column names (<code>--csv-no-header</code> names columns <code>col1..colN</code> in file order). Opening a csv/tsv source without either is refused loudly (<code>SLUICE-E-CSV-HEADER-UNDECLARED</code>). Mutually exclusive.</td></tr>
  <tr><td><code>--csv-null</code></td><td class="desc">The UNQUOTED field text that means SQL NULL: <code>--csv-null=''</code> (the PostgreSQL COPY CSV convention — an unquoted empty field is NULL), <code>--csv-null='\\N'</code>, or <code>--csv-null=NULL</code>. A QUOTED field is always data (<code>"NULL"</code> is the four-character string; <code>""</code> is the empty string). Without this flag a file containing an unquoted empty field is refused loudly (<code>SLUICE-E-CSV-NULL-AMBIGUOUS</code>) — a file with no empty unquoted fields needs no flag.</td></tr>
  <tr><td><code>--csv-delimiter</code></td><td class="desc"><code>csv</code> driver only: the field delimiter — a single ASCII character, or <code>\\t</code>/<code>tab</code> for TAB (default <code>,</code>). The <code>tsv</code> driver is fixed to TAB.</td></tr>
  </tbody></table>
...
${cmd(
  ...
  <tr><td><code>--source-tls-ca</code> / <code>--target-tls-ca</code></td><td class="desc">Path to a PEM CA certificate for <strong>CA-pinned verify-ca TLS</strong> to a MySQL source / target (ADR-0158): trust this CA, verify the server certificate chains to it, skip the hostname check — the strongest mode that works against MySQL's SAN-less auto-generated certs. On the source it covers both the data connection and the binlog/CDC stream. Refused if the DSN already sets <code>tls=</code>; refused on non-MySQL endpoints (Postgres uses <code>sslrootcert=/path/ca.pem</code> in the DSN instead). Also accepted by <code>sync start</code>, <code>verify</code>, <code>backup</code>, and <code>restore</code> (target side).</td></tr>
...
${cmd(
  ...
  <tr><td><code>--planetscale-org</code></td><td class="desc">PlanetScale org slug, consumed by <strong>both</strong> optional PlanetScale integrations — each arms on its own token pair (v0.99.259): <strong>(1) target-health telemetry</strong> (CPU/mem/storage/lag) read from the PlanetScale metrics endpoint (ADR-0107), feeding proactive apply back-off and the <code>sluice_target_*</code> gauges — opt-in and <strong>all-or-nothing</strong> with the metrics-token pair (org + a partial metrics pair is a loud refusal); <strong>(2) the ADR-0148 deploy-request index-build fallback</strong> for the cold-start index phase on a <code>planetscale</code> target — opportunistic, WARN-at-most, arming on the service-token pair (a fallback-only arming never trips the telemetry refusal). A <strong>control-plane credential, distinct from the data-plane <code>--target</code> DSN</strong>. Unlike migrate's flag it has no ambient <code>PLANETSCALE_ORG</code> env binding here — arming needs the explicit flag (the tokens still come from env). Off when unset (default sync unchanged).</td></tr>
  ...
  <tr><td><code>--planetscale-database</code> / <code>--planetscale-branch</code> / <code>--planetscale-service-token-id</code> / <code>--planetscale-service-token</code> / <code>--planetscale-deploy-timeout</code></td><td class="desc">ADR-0148 index-build fallback inputs — same set and defaults as <a href="/docs/commands/#migrate">migrate</a>'s: database defaults to the <code>--target</code> DSN's, branch to <code>main</code>, deploy deadline <code>1h</code>; the service token (branch + deploy-request scopes) via env <code>PLANETSCALE_SERVICE_TOKEN_ID</code> / <code>PLANETSCALE_SERVICE_TOKEN</code>. Before v0.99.259 the cold-start's walled PlanetScale index build always ended at the <code>SLUICE-E-INDEX-*</code> hint; armed, sluice builds it via a deploy request. An unarmed run is byte-identical.</td></tr>
  ...
  <tr><td><code>--notify-schema-drift</code></td><td class="desc"><strong>On by default</strong> (ADR-0157; inert unless a <code>--notify-*</code> sink is configured): fire a critical notification when a source schema change <strong>stalls the sync</strong> — a DDL sluice cannot auto-forward (e.g. <code>RENAME COLUMN</code> on MySQL). The alert carries the drift detail + recovery steps. <strong>Ungated</strong> from PlanetScale telemetry — works on every engine pair. Pass <code>--notify-schema-drift=false</code> to disable while keeping metrics alerts. Advisory + failure-isolated: a delivery problem never affects the (already stalled) sync.</td></tr>
  <tr><td><code>--notify-slot-health</code></td><td class="desc"><strong>On by default</strong> (ADR-0059; inert unless a sink is configured; fires only for Postgres logical-replication sources — the structured slog WARNs fire regardless): notify when the source replication slot crosses a health threshold — WAL retention pressure at 70% (warning) / 85% (critical) of <code>max_slot_wal_keep_size</code>, 30m slot inactivity (warning), <code>wal_status</code> <code>unreserved</code> (critical — invalidation at the next checkpoint), and the terminal events (<code>wal_status</code> <code>lost</code>, slot dropped mid-stream) each page critical exactly once and latch. A sustained slot-health probe outage (5 consecutive failures) pages a warning — the net never goes silently blind. Pass <code>--notify-slot-health=false</code> to keep the slog WARNs only. Advisory + failure-isolated.</td></tr>
  <tr><td><code>--source-tls-ca</code> / <code>--target-tls-ca</code></td><td class="desc">CA-pinned verify-ca TLS to a MySQL source / target (ADR-0158) — PEM CA path; covers the data connection <em>and</em> the binlog/CDC stream on the source. See the <a href="/docs/commands/#migrate">migrate</a> row for the full semantics.</td></tr>
...
${cmd(
  ...
  <tr><td><code>--sign</code></td><td class="desc">Sign the backup manifest + lineage catalog with a detached HMAC-SHA-256 keyed off the chain KEK (ADR-0154 Phase 1). Requires <code>--encrypt</code> with a passphrase (HMAC-off-KEK signs only encrypted chains); extending an already-signed chain signs automatically. Mutually exclusive with <code>--sign-key</code>.</td></tr>
...
<h2 id="backup-export-parquet">backup export-as-parquet</h2>
${cmd(
  "backup-export-parquet-c",
  ...
  `<p>The export represents <strong>one snapshot</strong> — the latest full by default, or the full named by <code>--backup-id</code>. Incremental change-windows after that full are <em>not</em> folded in (a loud WARN names the count); operators who need point-in-time state restore the chain and re-export. Exit-only: sluice never reads its Parquet output back — <code>sluice restore</code> keeps the JSON-Lines path. The Parquet files themselves are written <strong>plaintext</strong> even from an encrypted chain — the analytics destination's encryption posture is a separate operator choice.</p>
  <table><thead><tr><th>Flag</th><th>Purpose</th></tr></thead><tbody>
  <tr><td><code>--from-dir</code> / <code>--from</code></td><td class="desc">The backup to export: a local directory (the same one <code>--output-dir</code> wrote to), or a URL (<code>s3://</code>, <code>gs://</code>, <code>azblob://</code>, <code>file:///</code>). One is required; mutually exclusive.</td></tr>
  <tr><td><code>--output-dir</code> / <code>--output</code></td><td class="desc">Destination for the Parquet files + <code>parquet_index.json</code>: a local directory (created if absent) or a URL (<code>s3://bucket/prefix</code>, <code>gs://</code>, <code>azblob://</code>, <code>file:///</code>). One is required; mutually exclusive.</td></tr>
  <tr><td><code>--backup-endpoint</code> / <code>--backup-region</code> / <code>--backup-path-style</code></td><td class="desc">S3-compatible-provider overrides (endpoint, region, path-style addressing) — apply to <strong>both</strong> <code>--from</code> and <code>--output</code> when they are <code>s3://</code> URLs.</td></tr>
  <tr><td><code>--include-table</code> / <code>--exclude-table</code></td><td class="desc">Glob-aware table filters (comma-separated, repeatable; mutually exclusive) — export a subset of the snapshot's tables.</td></tr>
  <tr><td><code>--backup-id</code></td><td class="desc">Export the segment <strong>full</strong> snapshot with this BackupID instead of the latest one (chain-to-a-point at snapshot granularity; find ids in the chain's manifests or <code>lineage.json</code>). Incremental ids are refused — their change-windows are not exportable.</td></tr>
  <tr><td><code>--force-overwrite</code></td><td class="desc">Replace a prior export at the destination. By default the command refuses when <code>parquet_index.json</code> is already present.</td></tr>
  <tr><td><code>--encrypt</code> + key flags / <code>--verify-key</code> / <code>--require-signature</code></td><td class="desc">Read-side encryption + signature flags, mirroring <a href="/docs/commands/#restore">restore</a>: an encrypted chain needs <code>--encrypt</code> + the chain's passphrase / KMS reference; a signed chain is verified (strictly, with <code>--require-signature</code>) before any chunk is decoded.</td></tr>
  </tbody></table>
  ${pre(`sluice backup export-as-parquet --from s3://my-bucket/app-chain \\
    --output-dir ./warehouse-drop \\
    --exclude-table 'audit_*'`)}
  <p><strong>Never a silent narrow:</strong> a column type or value with no faithful Parquet representation — a multi-dimensional array, a TIME outside a calendar day (MySQL TIME reaches ±838h), a PG <code>NUMERIC</code> <code>NaN</code>/<code>Infinity</code>, a sub-microsecond timestamp — is refused loudly with <code>SLUICE-E-EXPORT-UNREPRESENTABLE</code> (exclude the table and export the rest, or query that table's JSON-Lines chunks directly — DuckDB reads them natively). The documented string downgrades (unbounded <code>NUMERIC</code>, <code>TIMETZ</code>) carry the exact value text and are WARNs, not refusals.</p>`
)}
...
${cmd(
  ...
  <tr><td><code>--encrypt</code> + key flags / <code>--verify-key</code> / <code>--require-signature</code></td><td class="desc">Read-side chain unwrapping and signature verification — the same flags the <a href="/docs/commands/#backup">backup</a> write side takes: an encrypted chain needs <code>--encrypt</code> + the chain's passphrase / KMS reference (a mismatched or missing key mode is refused at preflight with <code>SLUICE-E-BACKUP-ENCRYPTION-MISMATCH</code>); an asymmetrically-signed chain needs <code>--verify-key</code>, strict with <code>--require-signature</code>.</td></tr>
...
${cmd(
  ...
  `<p>Backfill is <strong>single-endpoint</strong> — it runs INSIDE one database (no source/target pair), walking the table's primary key in bounded batches and issuing one <code>UPDATE</code> per batch, so no statement ever locks (or hits the statement-time wall of a managed provider on) more than <code>--batch-size</code> rows. The cursor persists in the same database's <code>sluice_migrate_state</code> control tables, so a killed run resumes where it left off — the crash-replay window is at most one chunk, and the replayed chunk is a no-op under a self-describing <code>--where</code> guard. The <code>--set</code> expressions and the <code>--where</code> predicate are <strong>native SQL for the <code>--driver</code> engine, emitted verbatim</strong> (same-database, so there is no cross-dialect translation to do).</p>
  <table><thead><tr><th>Flag</th><th>Purpose</th></tr></thead><tbody>
  <tr><td><code>--driver</code></td><td class="desc"><strong>Required.</strong> Engine name for the database (<code>mysql</code>, <code>planetscale</code>, <code>vitess</code>, or <code>postgres</code> — the engines that implement the in-place backfill surface). SQLite/D1 refuse with <code>SLUICE-E-BACKFILL-UNSUPPORTED-ENGINE</code> (a single-file/edge database doesn't need the online-safety machinery — run the UPDATE directly).</td></tr>
  <tr><td><code>--dsn</code></td><td class="desc"><strong>Required.</strong> Database DSN. Backfill is same-database: it reads and updates this one endpoint.</td></tr>
  <tr><td><code>--table</code></td><td class="desc"><strong>Required.</strong> Table to backfill. It must have a usable orderable primary key to cursor on — a keyless table (or a JSON/array/geometry PK) refuses with <code>SLUICE-E-BACKFILL-NO-PRIMARY-KEY</code>; there is no flag to force an unbounded whole-table UPDATE.</td></tr>
  <tr><td><code>--set</code></td><td class="desc">Assignment <code>'COL = EXPR'</code> applied to every matched row (repeatable; required except with <code>--verify-only</code>). Split at the FIRST <code>=</code>, so expressions may themselves contain <code>=</code>. A <code>--set</code> column that doesn't exist on the table refuses up front with <code>SLUICE-E-BACKFILL-UNKNOWN-COLUMN</code> (the message lists the table's actual columns).</td></tr>
  <tr><td><code>--where</code></td><td class="desc">Native-SQL predicate scoping which rows are backfilled. Make it <strong>self-describing</strong> (e.g. <code>new_col IS NULL</code>) so re-runs and crash-resume skip already-done rows.</td></tr>
  <tr><td><code>--batch-size</code></td><td class="desc">Rows per bounded UPDATE batch (keyset-chunked walk of the primary key). <code>0</code> (default) uses sluice's bulk-copy default.</td></tr>
  <tr><td><code>--dry-run</code></td><td class="desc">Print the generated per-chunk UPDATE statement and an affected-row estimate, then exit without writing anything.</td></tr>
  <tr><td><code>--restart</code></td><td class="desc">Discard the stored resume cursor for this exact spec (<code>--set</code>/<code>--where</code>) and start over from the beginning of the table. Refused while another run of the same spec looks live (<code>SLUICE-E-BACKFILL-CONCURRENT-RUN</code>) — it would clear the state row out from under the live walker.</td></tr>
  <tr><td><code>--verify</code></td><td class="desc">After the run completes, count rows still matching <code>--where</code>: <code>0</code> prints the safe-to-contract signal; <code>&gt;0</code> fails with <code>SLUICE-E-BACKFILL-INCOMPLETE</code> (re-run to catch up, then verify again). Requires <code>--where</code>.</td></tr>
  <tr><td><code>--verify-only</code></td><td class="desc">Skip the walk and just run the <code>--where</code> remaining-count gate (no UPDATEs, no control-table writes) with the same 0 / &gt;0 exit contract — the scriptable post-migration check. Requires <code>--where</code>; <code>--set</code> is optional.</td></tr>
  </tbody></table>
  ${pre(`# expand step done (new column exists); backfill it online, then gate the contract step
sluice backfill --driver planetscale --dsn 'user:pass@tcp(host)/app' --table users \\
    --set "full_name = CONCAT(first_name, ' ', last_name)" \\
    --where 'full_name IS NULL' \\
    --verify`)}
  <div class="note"><strong>Resume vs restart.</strong> A killed run resumes from the persisted cursor automatically on re-run; a spec whose stored state is already <code>complete</code> needs <code>--restart</code> to walk again (the <code>SLUICE-E-BACKFILL-INCOMPLETE</code> catch-up loop). A cursor persisted by an older sluice whose JSON store mangled binary or &gt;2<sup>53</sup> integer PK values is refused loudly (<code>SLUICE-E-BACKFILL-CORRUPT-CURSOR</code>) rather than silently skipping PK ranges — re-run with <code>--restart</code>; a self-describing guard makes the re-walk touch only the rows the interrupted run never reached. A second invocation of the same spec while the first is still walking — its state-row heartbeat fresher than 5 minutes, typically an overlapping cron — is refused with <code>SLUICE-E-BACKFILL-CONCURRENT-RUN</code> (v0.99.260): two concurrent walks would interleave cursor writes and break the at-most-one-chunk replay bound. Heartbeat-only, no lease — a kill -9'd run keeps the spec refused for at most one window.</div>`
)}

<h2 id="expand-contract">expand-contract</h2>
${cmd(
  "expand-contract-c",
  ...
  `<p><strong>This command mutates a production branch.</strong> It is PlanetScale-specific by design: it needs the control-plane service token on top of the data-plane DSN, and the production branch must have <strong>safe migrations enabled</strong> — deploy requests are the mechanism the expand and contract legs ship through. sluice never flips that toggle for you: with safe migrations off it refuses with <code>SLUICE-E-PS-SAFE-MIGRATIONS-DISABLED</code>. The legs: <strong>expand</strong> creates a sluice dev branch, applies <code>--expand-ddl</code>, and deploys it via a deploy request (sluice's control tables ride inside the same deploy); <strong>migrate</strong> runs the <a href="/docs/commands/#backfill">backfill</a> against the production data with your <code>--set</code>/<code>--where</code>; <strong>verify</strong> re-counts the <code>--where</code> guard; <strong>contract</strong> — a destructive <code>DROP COLUMN</code> — runs <em>only</em> after a clean verify <strong>and</strong> <code>--yes</code>. Without <code>--yes</code> (or without <code>--contract-ddl</code>) the run stops after verify and prints the exact resume command, so the destructive leg is always an explicit second decision.</p>
  <table><thead><tr><th>Flag</th><th>Purpose</th></tr></thead><tbody>
  <tr><td><code>--org</code></td><td class="desc"><strong>Required</strong> (or env <code>PLANETSCALE_ORG</code>). PlanetScale organization slug.</td></tr>
  <tr><td><code>--database</code></td><td class="desc"><strong>Required.</strong> PlanetScale database name.</td></tr>
  <tr><td><code>--branch</code></td><td class="desc">Production branch the pattern targets (deploy requests merge into it; the backfill runs against its data). Default <code>main</code>.</td></tr>
  <tr><td><code>--service-token-id</code> / <code>--service-token</code></td><td class="desc">PlanetScale service token (branch + deploy-request scopes). Set via the env vars <code>PLANETSCALE_SERVICE_TOKEN_ID</code> / <code>PLANETSCALE_SERVICE_TOKEN</code> (the pscale CLI convention) — never on the command line; never logged. Required except under <code>--dry-run</code>, which makes no control-plane call.</td></tr>
  <tr><td><code>--dsn</code></td><td class="desc"><strong>Required.</strong> Data-plane MySQL DSN for the production branch — the migrate (backfill) leg runs inside it. The engine is fixed to <code>planetscale</code> (no <code>--driver</code> to mis-set).</td></tr>
  <tr><td><code>--table</code></td><td class="desc"><strong>Required.</strong> Table the pattern operates on.</td></tr>
  <tr><td><code>--expand-ddl</code></td><td class="desc">Verbatim ADD COLUMN DDL for the expand leg (e.g. <code>ALTER TABLE t ADD COLUMN full_name VARCHAR(255)</code>), applied on a dev branch and shipped via a deploy request. Required unless <code>--resume-from</code> skips the leg.</td></tr>
  <tr><td><code>--contract-ddl</code></td><td class="desc">Verbatim DROP COLUMN DDL for the contract leg. Optional: without it the run stops after verify with resume instructions. Runs only after a clean verify AND <code>--yes</code>.</td></tr>
  <tr><td><code>--set</code> / <code>--where</code></td><td class="desc">The backfill assignment(s) (repeatable; native SQL, emitted verbatim) and the self-describing guard (e.g. <code>new_col IS NULL</code>). <code>--where</code> is <strong>required</strong>: it scopes the backfill AND is the verify gate that authorizes the contract step.</td></tr>
  <tr><td><code>--batch-size</code></td><td class="desc">Rows per bounded backfill UPDATE. <code>0</code> (default) uses sluice's bulk-copy default.</td></tr>
  <tr><td><code>--yes</code>, <code>-y</code></td><td class="desc">Confirm the contract leg (a destructive DROP COLUMN deploy request). Without it the run stops after verify and prints the exact resume command.</td></tr>
  <tr><td><code>--dry-run</code></td><td class="desc">Print the full plan — branches, deploy requests, the rendered backfill statement, the gates — without a single control-plane call and without writing anything.</td></tr>
  <tr><td><code>--keep-branches</code></td><td class="desc">Keep the sluice dev branches instead of deleting them at the end (debugging aid).</td></tr>
  <tr><td><code>--resume-from</code></td><td class="desc">Leg to continue from after an interrupted run: <code>expand</code> (default, full pattern), <code>migrate</code> (the ADD COLUMN already deployed), <code>contract</code> (the backfill already completed; still re-verifies — <code>--set</code> is optional here).</td></tr>
  <tr><td><code>--poll-interval</code></td><td class="desc">Deploy-request / branch state polling cadence. Default <code>10s</code>.</td></tr>
  <tr><td><code>--deploy-timeout</code></td><td class="desc">Per-deploy-request deadline. Default <code>1h</code> — large tables deploy via VReplication: real wall-clock, but async and unbounded by errno 3024. A deploy request that outwaits the deadline still un-deployed <strong>keeps the dev branch</strong> (deleting it would close the still-open deploy request you were just told to approve); the timeout message names the kept branch and the post-close delete recipe (v0.99.260).</td></tr>
  </tbody></table>
  ${pre(`export PLANETSCALE_SERVICE_TOKEN_ID=...
...
sluice expand-contract --org acme --database app \\
    ...
    --yes`)}
  <div class="note"><strong>The SLUICE-E-PS-* refusal family names each failure precisely.</strong> <code>SLUICE-E-PS-SAFE-MIGRATIONS-DISABLED</code> (exit 3): safe migrations is off on the branch — enable it in the PlanetScale UI or via <code>pscale branch safe-migrations enable</code>; sluice never auto-enables a production-branch behavior change. <code>SLUICE-E-PS-DEPLOY-REQUEST-FAILED</code>: a deploy request errored, was closed, computed an empty or stranger-touching diff, or outran <code>--deploy-timeout</code> — the message carries the DR number, state, and URL, and a timed-out expand continues with <code>--resume-from migrate</code>. <code>SLUICE-E-PS-BRANCH-STALE-BASE</code>: a fresh PlanetScale dev branch's schema can <em>lag</em> production (observed live: a branch created 14 minutes after a deploy still lacked the deployed column), and a deploy request from a stale base would silently <strong>revert</strong> newer production schema — sluice gates every dev branch on freshness, self-heals once via an on-demand backup + branch re-create, and raises this only if still stale. Two more pre-deploy gates ship since ADR-0167 (v0.99.258): the deploy request's computed diff is refused if it touches any object the leg never intended (the stale-base phantom-revert signature), and after a review/deploy wait longer than ~2 minutes production's schema is re-verified against the provisioning baseline — refusing <code>SLUICE-E-PS-BRANCH-STALE-BASE</code> if it moved mid-wait.</div>
  <div class="note"><strong>Re-running the pattern on a reused database:</strong> when the current run's expand leg actually deployed a schema change, the backfill walk restarts even if a prior cycle left a completed marker for the identical <code>--set</code>/<code>--where</code> spec — the self-describing guard scopes the re-walk to unfinished rows; <code>--resume-from migrate</code> still honors mid-walk cursors and completed markers, and standalone <code>sluice backfill</code> is unchanged (v0.99.258).</div>`
)}

<h2 id="deploy-ddl">deploy-ddl</h2>
${cmd(
  "deploy-ddl-c",
  "sluice deploy-ddl",
  ...
  `<p><strong>This command mutates a production branch</strong> (through PlanetScale's governed deploy-request channel). It replaces five hand-driven <code>pscale</code> commands plus a hazard the operator can't see — a fresh PlanetScale dev branch can silently propose <em>reverting</em> recent production schema (the <code>SLUICE-E-PS-BRANCH-STALE-BASE</code> gate above catches it). It requires safe migrations ON the branch (the deploy-request prerequisite; without safe migrations, direct DDL works and this command is unnecessary). There is no data-plane DSN: the DDL runs on the dev branch via a just-minted branch password. The named consumer is the one-time <strong>control-table bootstrap</strong> on a safe-migrations branch — <a href="/docs/commands/#control-tables">control-tables ddl</a> prints the statements to ship.</p>
  <table><thead><tr><th>Flag</th><th>Purpose</th></tr></thead><tbody>
  <tr><td><code>--org</code></td><td class="desc"><strong>Required</strong> (or env <code>PLANETSCALE_ORG</code>). PlanetScale organization slug.</td></tr>
  <tr><td><code>--database</code></td><td class="desc"><strong>Required.</strong> PlanetScale database name.</td></tr>
  <tr><td><code>--branch</code></td><td class="desc">Production branch the deploy request merges into (must have safe migrations enabled). Default <code>main</code>.</td></tr>
  <tr><td><code>--service-token-id</code> / <code>--service-token</code></td><td class="desc">PlanetScale service token (branch + deploy-request scopes), via env <code>PLANETSCALE_SERVICE_TOKEN_ID</code> / <code>PLANETSCALE_SERVICE_TOKEN</code>; never logged. Required except under <code>--dry-run</code>.</td></tr>
  <tr><td><code>--ddl</code></td><td class="desc"><strong>Required.</strong> The single verbatim DDL statement to ship (e.g. <code>CREATE TABLE ...</code> or <code>ALTER TABLE ...</code>), applied on a dev branch exactly as written and deployed via a deploy request.</td></tr>
  <tr><td><code>--dry-run</code></td><td class="desc">Print the plan — branch name, the DDL, the deploy-request flow — without a single control-plane call and without writing anything.</td></tr>
  <tr><td><code>--keep-branches</code></td><td class="desc">Keep the sluice dev branch instead of deleting it at the end (debugging aid).</td></tr>
  <tr><td><code>--poll-interval</code></td><td class="desc">Deploy-request / branch state polling cadence. Default <code>10s</code>.</td></tr>
  <tr><td><code>--deploy-timeout</code></td><td class="desc">Deploy-request deadline. Default <code>1h</code> (large tables deploy via VReplication — async, unbounded by errno 3024). On a timeout with the deploy request still un-deployed the dev branch is <strong>kept</strong> — deleting it would close the still-open deploy request; the message names the kept branch and the cleanup recipe (v0.99.260). The same ADR-0167 post-wait freshness recheck as expand-contract guards a &gt;2-minute review wait (operator-authored DDL skips only the diff-scope assertion).</td></tr>
  </tbody></table>
  ${pre(`# bootstrap sluice's control tables on a safe-migrations branch:
...
<h2 id="control-tables">control-tables ddl</h2>
${cmd(
  "control-tables-c",
  ...
  `<p>Read-only, needs no credentials and no org/database — output is pure SQL plus <code>--</code> comment lines, so it pastes or pipes into any governed channel: <a href="/docs/commands/#deploy-ddl">deploy-ddl</a> (one statement per run), the PlanetScale UI, or a reviewed migration file. On a PlanetScale branch with safe migrations enabled, direct DDL is refused (Error 1105, surfaced as <code>SLUICE-E-PS-DIRECT-DDL-BLOCKED</code>) — sluice's own ensure paths are detect-first, so pre-creating the control tables this way lets <code>migrate</code> / <code>sync</code> / <code>backfill</code> run against the branch without ever needing a direct CREATE.</p>
  <table><thead><tr><th>Flag</th><th>Purpose</th></tr></thead><tbody>
  <tr><td><code>--engine</code></td><td class="desc">Engine whose control-table dialect to print. Default <code>planetscale</code> (the bootstrap consumer — safe migrations blocks direct DDL); <code>mysql</code> / <code>vitess</code> print the same dialect. Engines that don't publish their control-table DDL are refused by name.</td></tr>
  </tbody></table>
  ${pre(`sluice control-tables ddl                       # planetscale dialect (default)
...
// Ported nearly verbatim from sluice's docs/operator/error-codes.md, which is
// itself test-enforced (both directions) against the registry in
// internal/sluicecode/sluicecode.go. The build-time drift check at the bottom
// of this file counts these rows against a sibling sluice checkout's registry
// and fails the build on a mismatch, so a newly minted code can't silently go
// missing here.
...
<tr><td><code>SLUICE-E-INDEX-STATEMENT-TIME-LIMIT</code></td><td class="desc">runtime</td><td class="desc">A post-copy index build hit PlanetScale's statement-time limit (MySQL errno 3024); the data is already copied. This surfaces only when the automatic deploy-request fallback (ADR-0148) is unarmed or unavailable — armed (planetscale target + <code>--planetscale-org</code> + service token + safe migrations ON), sluice builds the index via a deploy request instead. The arming flags exist on <code>migrate</code>, <code>restore</code>, and <code>sync start</code> (the three modes that run the deferred index build) — not yet on the fleet <code>sync run</code> specs or the <code>sync from-backup</code> broker.</td><td class="desc"><code>--resume</code> finishes just the indexes with no re-copy (grow the PlanetScale cluster first for a faster build), start fresh with <code>--upfront-indexes</code>, or arm the deploy-request fallback (<code>--planetscale-org</code> + <code>PLANETSCALE_SERVICE_TOKEN_ID</code>/<code>_TOKEN</code>, safe migrations ON).</td></tr>
<tr><td><code>SLUICE-E-INDEX-DIRECT-DDL-DISABLED</code></td><td class="desc">runtime</td><td class="desc">PlanetScale safe-migrations is enabled on the target branch and blocked a direct <code>ADD INDEX</code> at the <strong>deferred index-build phase</strong> (errno 1105) — in practice a <code>--resume</code>/restore continuing an earlier copy, or a branch whose safe migrations was enabled mid-run. This surfaces only when the automatic deploy-request index-build fallback (ADR-0148) is unarmed — armed with <code>--planetscale-org</code> + a service token, sluice builds the indexes through a deploy request; the arming flags exist on <code>migrate</code>, <code>restore</code>, and <code>sync start</code>, not yet on the fleet <code>sync run</code> specs or the <code>sync from-backup</code> broker. Note the fallback covers ONLY the index phase: a <strong>fresh</strong> migrate into a safe-migrations branch refuses earlier, at user-table creation, with <code>SLUICE-E-PS-DIRECT-DDL-BLOCKED</code> — unless the schema was pre-created via deploy requests, in which case migrate's pre-create gate skips the matching tables (ADR-0166) and the deploy-request path carries a fresh migrate end to end.</td><td class="desc">Give the command (<code>migrate</code>, <code>restore</code>, or <code>sync start</code>) a PlanetScale service token (<code>--planetscale-org</code> + <code>PLANETSCALE_SERVICE_TOKEN_ID</code>/<code>_TOKEN</code> env) so the deploy-request fallback engages, or disable safe-migrations on the branch for the migration (the standalone <a href="/docs/commands/#expand-contract"><code>sluice expand-contract</code></a> and <a href="/docs/commands/#deploy-ddl"><code>sluice deploy-ddl</code></a> commands ship individual schema changes via deploy requests too).</td></tr>
...
<tr><td><code>SLUICE-E-INDEX-MISSING</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">The post-copy verification found the target is missing one or more secondary indexes the migration was expected to build (named as <code>table.index</code> in the message) — a loud-failure safety net against a silent index-build no-op. sluice refuses to report a successful migration with an incomplete schema.</td><td class="desc">Re-run with <code>--resume</code> to rebuild the missing indexes; if it recurs, the target rejected the index DDL — check the target's DDL/online-migration policy and the logs for the underlying error.</td></tr>
<tr><td><code>SLUICE-E-VSTREAM-FLOAT-LOSSY</code></td><td class="desc"><strong>refusal</strong></td><td class="desc"><code>backup full</code> on a PlanetScale/Vitess (VStream) source with <code>--strict-float</code>, when a single-precision <code>FLOAT</code> column cannot be re-read exactly: the table is <strong>keyless / float-PK-only</strong> (no primary key to target the exact re-read — refused upfront), <strong>larger than <code>--float-reread-max-rows</code></strong> (too large for the bounded-memory exact re-read), or the exact re-read returned rows but <strong>not one streamed row's primary key matched</strong> (a systemic PK-rendering divergence). vttablet's rowstreamer renders FLOAT at mysqld's 6-significant-digit display precision, and <code>--strict-float</code> demands exact-or-fail.</td><td class="desc">Add a primary key (or exclude the table), raise <code>--float-reread-max-rows</code> if it's a size cap and you have the headroom, or drop <code>--strict-float</code> (the default archives exact where it can and rounded — with a WARN — elsewhere). A target-side <code>--type-override</code> to DOUBLE does NOT help — the source value is already rounded on the wire.</td></tr>
<tr><td><code>SLUICE-E-BACKUP-SIGNATURE-INVALID</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A signed (FormatVersion 6, ADR-0154) backup manifest's detached signature failed verification — the manifest was tampered, rolled back to an older version, its change-list was truncated, the wrong key was supplied, or the signature scheme/algorithm was relabeled. Applies to all three signing schemes: HMAC-off-KEK (<code>--sign</code>, Phase 1), Ed25519 (<code>--sign-key &lt;pem&gt;</code>/<code>--verify-key &lt;pem&gt;</code>, Phase 2), and KMS (<code>--sign-key kms://...</code>/<code>--verify-key</code>, Phase 3). sluice refuses to restore/verify it before any data lands.</td><td class="desc">Restore from an untampered copy of the backup; if the whole store is suspect, the signature caught exactly the substitution it exists to catch. Verify the key matches the chain — the chain's <code>--encrypt</code> passphrase for an HMAC-signed chain, the correct <code>--verify-key</code> (a public-key PEM for Ed25519 / KMS, or <code>kms://...</code> to fetch the trusted key) for an asymmetrically-signed chain (the KEK does NOT verify an asymmetric signature). The recorded manifest key reference is never trusted — verification anchors on the key you supply.</td></tr>
...
<tr><td><code>SLUICE-E-BACKUP-SIGNATURE-UNSUPPORTED</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A signed backup manifest records a signature scheme FAMILY (e.g. a future post-quantum scheme) or a <code>kms/&lt;algorithm&gt;</code> whose algorithm this build of sluice does not know how to verify, or a canonicalization version newer than this build supports — the signature was written by a NEWER sluice. This is a forward-incompatibility, NOT a tamper signal: sluice fails closed (it will not restore/verify what it cannot check) but does not claim the backup is compromised. Distinct from <code>-INVALID</code> precisely so a version/scheme gap is never mistaken for an attack.</td><td class="desc">Upgrade sluice to a build that supports the backup's signature scheme/canonicalization, then re-run the restore/verify. The backup is not necessarily tampered — an older binary simply cannot verify a newer signature format.</td></tr>
<tr><td><code>SLUICE-E-BACKUP-MANIFEST-INVALID</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A backup manifest — or the chain of manifests — fails an internal-consistency check, refused before any data lands. Three shapes: (1) the recorded <code>BackupID</code> does not match its content, recomputed at restore/broker time from the fields the id deterministically covers (<code>created_at</code> / <code>source_engine</code> / <code>kind</code> / <code>EndPosition</code>); (2) the recorded <code>SchemaHash</code> does not match the manifest's own schema, recomputed at chain-restore time; (3) a mixed-mode lineage, where a segment's full and one of its incrementals disagree on encryption (one encrypted, one plaintext) — a mis-stitched or tampered chain. All three catch bit-rot, truncated rewrites, or a lazy tamper that edited a covered field without recomputing its fingerprint. They are corruption backstops, not tamper-proofing: a fully-coherent edit that also recomputes the fingerprints and fixes the parent-link chain is the signed-only boundary.</td><td class="desc">Restore from an untampered copy of the backup. To close the coherent-edit residual and catch this at <code>backup verify</code> time, sign the chain (<code>--sign</code> / <code>--sign-key</code> + <code>--require-signature</code>). Genuine corruption from a bad upload produces the same refusal; re-fetch/re-stream the manifest.</td></tr>
<tr><td><code>SLUICE-E-BACKUP-INCOMPLETE</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A restore, broker replay, or <code>export-as-parquet</code> decoded/applied a DIFFERENT number of changes/rows than the manifest records — the signing-independent backstop against silent truncation or edit of an unsigned backup (ADR-0154 R2 residual). Three shapes: (1) after replaying an incremental's change chunks the last applied change did not REACH the manifest's <code>EndPosition</code> (a store adversary dropped the tail change-chunk entries; survivors keep their ordinals so every GCM AAD still validates, but the intact <code>EndPosition</code> would overstate the data and poison a resumed CDC stream); (2) a table's/chunk's decoded row total disagrees with the manifest's recorded positive <code>RowCount</code> (the layer-2 count check, on restore and on Parquet export alike); (3) a full's table or chunk records 0 rows yet decodes rows (a zeroed <code>RowCount</code> that would otherwise disable the layer-2 backstop).</td><td class="desc">Restore/export from an untampered copy of the backup. To catch this class at <code>backup verify</code> time (before a restore) and to close the residual coherent-edit boundary, sign the chain (<code>--sign</code> / <code>--sign-key</code>) so the manifest signature covers the change-chunk ordinal + count + positions — an unsigned encrypted incremental is protected against tail-truncation only by this replay-time backstop. Genuine truncation from a bad upload produces the same refusal; re-fetch/re-stream the incremental.</td></tr>
<tr><td><code>SLUICE-E-BACKUP-CHUNK-AUTH-FAILED</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">An encrypted backup chunk failed its AES-GCM authenticated-decryption check during restore (or broker replay): the ciphertext or its bound additional-authenticated-data (AAD) does not match what was sealed — a tampered/corrupt chunk, or a spliced/reordered store where a chunk was moved between positions or between two same-column-set tables. This is the loud, coded TWIN of <code>SLUICE-E-BACKUP-SIGNATURE-INVALID</code> for a backup that is ENCRYPTED but NOT SIGNED: without a signature, sluice catches the swap/tamper at decrypt rather than at verify, and refuses before any row lands. By the time a chunk decrypts the chain key has already unwrapped (its wrap is itself authenticated), so this is never a wrong-passphrase error — a wrong key is caught earlier at the key unwrap.</td><td class="desc">Restore from an untampered copy of the backup; if the whole store is suspect, the AAD binding caught exactly the chunk substitution/splice it exists to catch. If you also want tamper caught at <code>backup verify</code> time (before a restore), sign the chain (<code>--sign</code> / <code>--sign-key</code>) — an unsigned encrypted backup is protected only at decrypt. Genuine bit-rot (not tampering) produces the same refusal; re-fetch the chunk object or restore from a healthy replica.</td></tr>
<tr><td><code>SLUICE-E-BACKUP-CHUNK-CORRUPT</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A backup chunk's STORED bytes do not hash to the SHA-256 recorded for it in the manifest, checked by rehashing the bytes during restore, broker replay, or <code>backup verify</code>. This is the byte-level integrity check that runs BEFORE decryption, so it fires on plaintext and encrypted chunks alike — the integrity TWIN of <code>SLUICE-E-BACKUP-CHUNK-AUTH-FAILED</code> (which is the AES-GCM authenticated-decryption check). It catches at-rest corruption / bit-rot and any tamper that altered a chunk's stored bytes (for an encrypted chunk a byte flip is caught here first, before the GCM tag). sluice refuses before any row lands.</td><td class="desc">Restore from an untampered / healthy copy of the backup, or re-fetch the chunk object from the store (a bad upload or storage bit-rot produces the same refusal as a tamper). If you want tamper caught at <code>backup verify</code> time and covered by a manifest signature, sign the chain (<code>--sign</code> / <code>--sign-key</code> + <code>--require-signature</code>).</td></tr>
<tr><td><code>SLUICE-E-BACKUP-CHAIN-CONFLICT</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">Another writer advanced this backup chain's lineage while this operation (a <code>backup full</code>/<code>backup incremental</code> finalize, a <code>backup stream</code> rollover, a rotation COMMIT, <code>backup compact</code>, <code>backup prune</code>, or <code>backup verify --rebuild-catalog</code>) was in flight. Every chain writer read-modify-writes the shared <code>lineage.json</code> catalog; two writers interleaving it (a duplicate cron entry running <code>backup incremental</code> twice, a backup racing a compact/prune, an operator double-start) can silently corrupt the chain's structure. The catalog write is a compare-and-swap on the chain's write-generation, so the losing writer refuses loudly and writes NO catalog change; the named marker records the other writer's host/pid/claim-time. Detection requires a store with conditional-write support (local <code>--output-dir</code>, S3, GCS, Azure; an S3-compatible endpoint that lacks conditional PUTs degrades to the old unguarded behavior with a WARN).</td><td class="desc">Check for a duplicate cron/scheduler entry or a concurrent backup/compact/prune/stream against the same chain; let the other writer finish, then re-run — the refused operation left the catalog untouched (a refused backup's own manifest is durable and is re-cataloged by the next writer or stream resume). If conflicts persist, inspect the newest <code>lineage.gen/</code> marker to identify the competing host/pid.</td></tr>
...
<tr><td><code>SLUICE-E-BACKFILL-NO-PRIMARY-KEY</code></td><td class="desc"><strong>refusal</strong></td><td class="desc"><code>sluice backfill</code> refused the table: it has no primary key, or a primary-key column is non-orderable (JSON/array/geometry) — so the keyset walk that bounds each UPDATE to <code>--batch-size</code> rows has nothing safe to cursor on. An unbounded whole-table UPDATE is exactly the statement-time-wall / long-lock shape the command exists to avoid, so sluice refuses rather than degrade to it.</td><td class="desc">Add a primary key to the table (or fix the non-orderable key), then re-run. There is no flag to force an unbounded backfill — for a small table where a single UPDATE is fine, run it directly in your SQL client.</td></tr>
<tr><td><code>SLUICE-E-BACKFILL-UNSUPPORTED-ENGINE</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">The <code>--driver</code> engine does not implement the in-place backfill surface. Backfill ships for MySQL (including the <code>planetscale</code> / <code>vitess</code> flavors, which ride the same bounded-UPDATE path) and Postgres; SQLite/D1 have no backfill executor.</td><td class="desc">Pass <code>--driver mysql</code>, <code>--driver planetscale</code>, <code>--driver vitess</code>, or <code>--driver postgres</code>. For SQLite/D1, run the transform directly — a single-file/edge database doesn't need the online-safety machinery.</td></tr>
<tr><td><code>SLUICE-E-BACKFILL-UNKNOWN-COLUMN</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A <code>--set</code> clause names a column that does not exist on the target table, caught against the read schema before any UPDATE runs (the message lists the table's actual columns). Refusing up front beats a per-chunk SQL error — or worse, a typo silently creating the impression the backfill ran.</td><td class="desc">Fix the <code>--set</code> column spelling (the left side of the first <code>=</code>); expressions on the right side are passed to the engine verbatim and are validated by the database itself.</td></tr>
<tr><td><code>SLUICE-E-BACKFILL-INCOMPLETE</code></td><td class="desc">runtime</td><td class="desc"><code>sluice backfill --verify</code> (or <code>--verify-only</code>) counted rows still matching the <code>--where</code> guard AFTER the walk completed — the check ran truthfully and found unfinished work, the online-backfill catch-up signal: rows inserted behind the walk's cursor during the run, or written since a completed run. The walk's own work is intact and persisted (the migration state stays <code>complete</code>); the gate is saying the table is not yet safe for the contract step.</td><td class="desc">Re-run the backfill to pick up the stragglers (a spec whose stored state is <code>complete</code> needs <code>--restart</code> to walk again), then verify again. On a quiesced database, a nonzero count after a clean walk means the <code>--where</code> guard does not actually self-describe doneness — fix the predicate (e.g. <code>new_col IS NULL</code>) so an already-backfilled row no longer matches.</td></tr>
<tr><td><code>SLUICE-E-BACKFILL-CORRUPT-CURSOR</code></td><td class="desc"><strong>refusal</strong></td><td class="desc"><code>sluice backfill</code> refused to resume: the persisted cursor for this spec was written by an older sluice whose JSON state store mangled non-string-safe PK values — binary cursor bytes had invalid-UTF-8 sequences replaced with U+FFFD, and integer cursors above 2<sup>53</sup> drifted through float64 — so its stored value provably no longer names the row the walk stopped at. Resuming from it would silently skip (or replay far beyond the one-chunk bound) arbitrary PK ranges, and a later contract step would destroy the never-backfilled rows; refusing loudly is the only safe answer. Current releases store cursors in a lossless tagged envelope, so fresh runs never produce this.</td><td class="desc">Re-run the same spec with <code>--restart</code> to walk the table from the beginning. With a self-describing <code>--where</code> guard (e.g. <code>new_col IS NULL</code>) the re-walk updates only the rows the interrupted run never reached — already-done rows are untouched.</td></tr>
<tr><td><code>SLUICE-E-BACKFILL-CONCURRENT-RUN</code></td><td class="desc"><strong>refusal</strong></td><td class="desc"><code>sluice backfill</code> refused to start: the spec's state row is still in the walking phase AND its heartbeat (the row's <code>updated_at</code>, touched on every committed chunk) is fresher than the 5-minute freshness window — another run of the same spec looks live, typically an overlapping cron invocation. Two concurrent walks of one spec interleave cursor writes, breaking the at-most-one-chunk replay bound into arbitrary skipped or replayed PK ranges, so sluice refuses before touching anything (including a <code>--restart</code>, which would clear the state row out from under the live walker). Heartbeat-only, no lease: a single chunk UPDATE outliving the window can slip past the guard, and a kill -9'd run keeps the spec refused for at most one window.</td><td class="desc">Wait for the running backfill to finish (or for its heartbeat to go stale — the window passes with no committed chunk), then re-run; the second invocation resumes from the persisted cursor. If no other run exists (e.g. the previous one was killed seconds ago), simply wait out the window. Deep client&harr;database clock skew can also trip the guard — it only ever over-refuses, never lets a concurrent walk through.</td></tr>
<tr><td><code>SLUICE-E-PS-SAFE-MIGRATIONS-DISABLED</code></td><td class="desc"><strong>refusal</strong></td><td class="desc"><code>sluice expand-contract</code> (or <code>sluice deploy-ddl</code>) refused: the PlanetScale production branch does not have safe migrations enabled, and deploy requests — the mechanism these commands ship schema changes through — cannot be created into such a branch. sluice never enables the toggle for you: it is a behavior change on your production branch (direct DDL becomes blocked from then on, and the enable/disable propagation lag makes toggling around a run unsafe). (<code>migrate</code>'s ADR-0148 index-build fallback never raises this code — with safe migrations off it simply stands down and the direct index error surfaces with its usual hint.)</td><td class="desc">Enable the branch's "Safe migrations" setting in the PlanetScale UI (or <code>pscale branch safe-migrations enable &lt;db&gt; &lt;branch&gt; --org &lt;org&gt;</code>), understanding that every future schema change on the branch must then ship via a deploy request; then re-run.</td></tr>
<tr><td><code>SLUICE-E-PS-DEPLOY-REQUEST-FAILED</code></td><td class="desc">runtime</td><td class="desc">A PlanetScale deploy request driven by sluice (<code>sluice expand-contract</code>, <code>sluice deploy-ddl</code>, or <code>migrate</code>'s ADR-0148 index-build fallback) entered a failure state (<code>error</code>, <code>complete_error</code>, <code>cancelled</code>, …), was closed without deploying, computed an empty diff (<code>no_changes</code> — that leg's DDL is likely already deployed from an earlier run), computed a diff touching an object the leg never intended (the ADR-0167 pre-deploy blast-radius check: a stranger table in the diff means the dev branch's base was stale or the branch was edited outside sluice — deploying would ship those changes; deploy-ddl carries no intended set, so this check applies to expand-contract and the index fallback), or did not become deployable / complete before <code>--deploy-timeout</code>. The message names the leg, the deploy-request number, the state it saw, and the deploy request's URL.</td><td class="desc">Inspect the deploy request at the URL in the message. A timeout on a large table usually means the VReplication deploy is still running — watch it finish, then continue with <code>--resume-from migrate</code> (after expand) or re-run the contract leg. A <code>no_changes</code> diff means that leg's DDL is already deployed: delete the leftover dev branch and resume past the leg. If your organization requires deploy-request approval, approve it and re-run — on that specific timeout sluice KEEPS the dev branch (deleting it would close the still-open deploy request); delete the branch yourself once the request closes, as the message spells out. For <code>migrate</code>'s index-build fallback, recovery is always <code>--resume</code>: the index phase re-probes the target and rebuilds only what is still missing, so an index that deployed after the timeout is simply detected and skipped.</td></tr>
...
<tr><td><code>SLUICE-E-PS-DIRECT-DDL-BLOCKED</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">The PlanetScale branch has safe migrations enabled, which refuses every direct DDL statement (Error 1105 "direct DDL is disabled") — and sluice needed one. Two cases, each named in the message: <strong>(a)</strong> creating (or column-migrating) one of sluice's own control tables — <code>sluice_migrate_state</code>, <code>sluice_cdc_state</code>, and siblings — where the ensure paths are detect-first, so this fires only when the DDL is genuinely needed and the message echoes the exact refused statement; <strong>(b)</strong> a <strong>user-table CREATE</strong> during <code>migrate</code>'s or sync cold-start's schema-apply phase — a fresh migrate into a safe-migrations branch always refuses here, before any data moves (the ADR-0148 index-build fallback engages later, at the index phase, and cannot help with table creation).</td><td class="desc">Case (a): bootstrap the control tables through the governed channel — <a href="/docs/commands/#control-tables"><code>sluice control-tables ddl</code></a> prints the exact CREATE statements, <a href="/docs/commands/#deploy-ddl"><code>sluice deploy-ddl --org &lt;org&gt; --database &lt;db&gt; --ddl '&lt;statement&gt;'</code></a> ships each one via a deploy request; then re-run. Case (b): disable safe migrations on the branch for the migration window and re-enable it after, or pre-create the schema via deploy requests (<code>sluice schema preview</code> prints the target DDL, <code>sluice deploy-ddl</code> ships each statement) — a re-run of <code>sluice migrate</code> then skips the pre-created tables whose column shape matches (ADR-0166), and a sync stream skips schema-apply with <code>sluice sync start --schema-already-applied</code>.</td></tr>
<tr><td><code>SLUICE-E-SOURCE-FOREIGN-DUMP</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">The <code>--source</code> file is a plain mysqldump / pg_dump <code>.sql</code> dump or a pg_dump custom-format (<code>PGDMP</code>) archive — full-dialect / private formats sluice deliberately does not parse (the IR-first tenet). Detected by content signature at open, on any file-reading source driver (<code>sqlite</code>, <code>mydumper</code>, <code>csv</code>/<code>tsv</code>/<code>ndjson</code>), before any data moves.</td><td class="desc">Restore the dump to a scratch server with its native tool and migrate live — the error message carries the exact three-command recipe (<code>docker run</code> a scratch MySQL/PostgreSQL, <code>mysql</code>/<code>psql</code>/<code>pg_restore</code> the dump into it, <code>sluice migrate</code> from the scratch server). A mydumper/<code>pscale database dump</code> DIRECTORY needs no scratch server: <code>--source-driver mydumper</code>.</td></tr>
<tr><td><code>SLUICE-E-SOURCE-WRONG-DRIVER</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">The <code>--source</code> is a recognisable input this source driver does not read: a mydumper directory handed to <code>csv</code>/<code>sqlite</code>, a CSV/TSV/NDJSON file handed to <code>mydumper</code>/<code>sqlite</code>, a binary SQLite <code>.db</code> handed to <code>csv</code>, a gzip/zstd-compressed file, a UTF-16 text file, a single-array JSON document handed to <code>ndjson</code>, or a <code>.tsv</code>/<code>.csv</code> extension contradicting the chosen delimited driver. Refused loudly instead of mis-parsing (a tab file through the comma lexer would silently stage one wide column).</td><td class="desc">The message names the right <code>--source-driver</code> or the preparation step (decompress, transcode to UTF-8, <code>jq -c '.[]'</code> for a JSON array). For a correctly-delimited file whose extension merely lies, declare intent with <code>--source-driver csv --csv-delimiter=...</code>.</td></tr>
<tr><td><code>SLUICE-E-CSV-NULL-AMBIGUOUS</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A <code>csv</code>/<code>tsv</code> source contains an UNQUOTED empty field and no NULL representation was declared. RFC 4180 has no NULL — NULL-vs-empty-string is pure producer convention and the #1 silent-loss class for CSV ingest — so sluice refuses to guess, naming the record and column. A QUOTED empty field (<code>""</code>) is unambiguously the empty string and never triggers this.</td><td class="desc">Declare the file's convention: <code>--csv-null=''</code> (an unquoted empty field means NULL — the PostgreSQL COPY CSV convention), or <code>--csv-null='\\N'</code> / <code>--csv-null=NULL</code> (that literal means NULL; empty fields are then empty strings). A quoted field is always data regardless.</td></tr>
<tr><td><code>SLUICE-E-CSV-HEADER-UNDECLARED</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A <code>csv</code>/<code>tsv</code> source was opened without <code>--csv-header</code> or <code>--csv-no-header</code>. Header presence is never sniffed: guessing wrong either silently eats the first data row as column names or turns the header into a data row.</td><td class="desc">Pass <code>--csv-header</code> (the first record carries the column names) or <code>--csv-no-header</code> (columns are named <code>col1..colN</code> in file order).</td></tr>
<tr><td><code>SLUICE-E-EXPORT-UNREPRESENTABLE</code></td><td class="desc"><strong>refusal</strong></td><td class="desc"><code>sluice backup export-as-parquet</code> refused a column type or value that has no faithful Parquet representation: a multi-dimensional array value (the column type declares no dimensionality, so the derived <code>LIST&lt;element&gt;</code> schema cannot hold nested lists), a TIME value outside a calendar day (MySQL TIME durations reach ±838h; PG allows <code>24:00:00</code>), a PG <code>NUMERIC</code> <code>NaN</code>/<code>Infinity</code> (Parquet DECIMAL has no non-finite form), a decimal with more digits than its declared precision/scale, or a timestamp with sub-microsecond precision. The export never silently narrows, rounds, or wraps a value — the documented string downgrades (unbounded <code>NUMERIC</code>, <code>TIMETZ</code>) carry the exact value text and are WARNs, not this refusal.</td><td class="desc">Exclude the affected table (<code>--exclude-table</code>) and export the rest, or query that table's JSON-Lines chunks directly (DuckDB reads them natively — see the <a href="https://github.com/sluicesync/sluice/blob/main/docs/cookbook/duckdb-on-sluice-backups.md">DuckDB cookbook</a>).</td></tr>
`;
...
<tr><td><code>2</code></td><td class="desc"><strong>Config error:</strong> the <code>--config</code> file could not be loaded or parsed. (The read-side commands <code>verify</code>/<code>diff</code>/<code>sync-health</code>/<code>metrics-watch</code> have always used 2 more broadly for "the check could not run at all". For <code>verify</code> this includes a run that completed but could not verify one or more tables — a per-table count/sample error, or a source table missing on the target: an unverified table is not a pass, so those runs exit 2 rather than a misleading 0. Tables deliberately excluded via <code>--include-table</code>/<code>--exclude-table</code> or config filters stay exit-neutral.)</td></tr>
...
<table><thead><tr><th>Code</th><th>Class</th><th>Meaning</th><th>Remedy</th></tr></thead><tbody>${ERROR_CODE_ROWS}</tbody></table>
`,
...
<tr><td><code>FLOAT</code> / <code>DOUBLE</code></td><td class="desc"><code>real</code> / <code>double precision</code></td><td class="desc">Finite values ride through exactly. <code>NaN</code>/±<code>Inf</code> are Postgres-only, so PG&rarr;MySQL refuses them loudly with <code>SLUICE-E-VALUE-UNREPRESENTABLE</code> (MySQL has no non-finite floats) rather than coerce them; MySQL&rarr;PG never produces one.</td></tr>
...
<table><thead><tr><th>Source ↓ &nbsp;/&nbsp; Target →</th><th>MySQL</th><th>PlanetScale / Vitess</th><th>Postgres</th><th>SQLite</th></tr></thead><tbody>
  ...
  <tr><td><strong>CSV / TSV / NDJSON</strong> (file, <code>csv</code>/<code>tsv</code>/<code>ndjson</code>)</td><td class="desc">✓</td><td class="desc">✓ <sup>b</sup></td><td class="desc">✓</td><td class="desc">✓</td></tr>
  <tr><td><strong>mydumper / pscale dump</strong> (directory, <code>mydumper</code>)</td><td class="desc">✓</td><td class="desc">✓ <sup>b</sup></td><td class="desc">✓</td><td class="desc">✓</td></tr>
</tbody></table>
...
<div class="note"><strong>Targets are MySQL, PlanetScale / Vitess, Postgres, or SQLite.</strong> Cloudflare D1 is a migrate <em>source</em> only (read live over its HTTP API), never a migrate target. The flat-file engines — <code>csv</code>, <code>tsv</code>, <code>ndjson</code> (each staged byte-exact into a temp SQLite database; file conventions declared with the <code>--csv-*</code> flags, never sniffed — ADR-0163) and <code>mydumper</code> (a mydumper / <code>pscale database dump</code> directory, ADR-0161) — are migrate <em>sources</em> only. Plain mysqldump / pg_dump <code>.sql</code> dumps are deliberately not parsed (loud refusal with a scratch-server recipe). The trigger-CDC engines (<code>postgres-trigger</code>, <code>sqlite-trigger</code>, <code>d1-trigger</code>) are sync-only and don't appear here.</div>
...
<ul>
  ...
  <li><strong><code>FormatVersion=5</code></strong> — an <strong>encrypted</strong> manifest (<code>--encrypt</code>, v0.99.202+). Its row chunks are AES-256-GCM ciphertext; an older binary that predates encryption refuses rather than mis-reading them.</li>
  <li><strong><code>FormatVersion=6</code></strong> — a <strong>signed</strong> encrypted manifest (<code>--sign</code>, v0.99.208+). The manifest carries a signature over its canonical bytes; a binary that can't verify it refuses rather than restoring an unverified signed chain.</li>
  <li><strong><code>FormatVersion=7</code></strong> — an <strong>encrypted</strong> manifest whose row chunks additionally bind their <em>parent table</em> into the GCM associated data (v0.99.214 signed / v0.99.219 unsigned), closing a store-adversary chunk-reassignment attack between two same-column-set tables.</li>
  <li><strong><code>FormatVersion=8</code></strong> — a <strong>CDC-segment</strong> manifest (incremental / streaming) from a <strong>VStream</strong> source (PlanetScale/Vitess) that folds its position-semantics flag into the deterministic <code>BackupID</code> (v0.99.228+). Only VStream CDC segments are stamped 8; full backups and non-VStream segments keep their feature-minimum version. An older binary refuses a v8 manifest at preflight rather than recompute-mismatching its id.</li>
</ul>
<p><code>FormatVersion=3</code> is a special case: it marks an <em>in-progress</em> full backup in the sidecar-checkpoint layout (v0.99.39+) and is <strong>never stamped on a finalized manifest</strong>. A finalized manifest carries the minimum version safe for its contents: a plaintext full is 1, 2, or 4 by schema; an encrypted/signed/table-bound chain rises to 5–7; and a VStream CDC segment is 8. It exists so an older binary refuses to <em>resume</em> an in-progress backup it can't account for, rather than mis-resuming off a base manifest that under-reports progress.</p>
...
// ---- drift check against a sibling sluice checkout -------------------------
// When a sibling sluice checkout exists next to this repo, compare the site's
// reference surface against the code's ground truth: (a) the error-codes
// table's row count vs the SLUICE-E-* registry in internal/sluicecode, and
// (b) the stated engine count vs the engines.Register(...) calls. A mismatch
// fails the build naming both counts, so a release that mints a code or
// registers an engine can't leave this site silently stale. No sibling
...
function driftCheck() {
  ...
  // (b) stated engine count vs engines.Register(...) calls (non-test files).
  let registerCalls = 0;
  const walk = (dir) => {
    for (const name of readdirSync(dir)) {
      ...
      if (statSync(p).isDirectory()) walk(p);
        ...
        // Skip //-comment lines: registry.go carries an engines.Register(...)
        // usage example in its doc comment.
        const code = readFileSync(p, "utf8").split("\n").filter((l) => !l.trimStart().startsWith("//")).join("\n");
        registerCalls += (code.match(/engines\.Register\(/g) || []).length;
      }
    }
  };
  ...
  console.log(`drift check OK: ${registered} error codes, ${registerCalls} engines — site matches the sibling sluice checkout`);
}
driftCheck();
