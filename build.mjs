...
<tr><td><code>SLUICE-E-VALUE-UNREPRESENTABLE</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">A source value has no representable target-type equivalent, so sluice refuses before the driver rather than corrupt it or retry-loop on a misleading server error — today: a <code>NaN</code>/±<code>Infinity</code> float into a MySQL <code>FLOAT</code>/<code>DOUBLE</code> (MySQL has no non-finite floats), or an <code>infinity</code>/pre-Gregorian (<code>BC</code>) Postgres timestamp into a fixed-width target.</td><td class="desc">Filter or transform the source value (e.g. <code>NULLIF</code> / <code>CASE</code> on the source query).</td></tr>
<tr><td><code>SLUICE-E-BACKUP-MANIFEST-INVALID</code></td><td class="desc"><strong>refusal</strong></td><td class="desc">At restore or broker apply, a manifest's recorded <code>BackupID</code> does not match the id recomputed from its content (<code>created_at</code>/<code>source_engine</code>/<code>kind</code>/<code>EndPosition</code>, plus the CDC-position flag on FormatVersion 8) — a corrupt or lazily-edited manifest, caught before any data lands. A corruption backstop, not tamper-proofing; sign chains for that.</td><td class="desc">Restore from an untampered copy, or sign the chain (<code>--sign</code> + <code>--require-signature</code>).</td></tr>
</tbody></table>
`,
