...
Index-column collations (COLLATE NOCASE / RTRIM) are carried, and a key the target cannot enforce is refused before any data moves (v0.155.0). SQLite compares an index column under the collation it was declared with, so email TEXT COLLATE NOCASE UNIQUE refuses 'A@X' after 'a@x'; sluice reads that per column from PRAGMA index_xinfo (BINARY, the default, is never carried, and the entry's DESC comes from the same catalog). A SQLite target emits the COLLATE verbatim and enforces the same key. Postgres and MySQL have no spelling for NOCASE (ASCII-only case folding) or RTRIM, so on a key that enforces uniqueness — a PRIMARY KEY, a UNIQUE, or a constraint-backed unique — the migration is refused at preflight, before any data moves, naming the column and the collation. The alternative was a BINARY unique on the target that admits rows the source rejects, at exit 0. To carry such a table, reproduce the case-insensitive key on the target yourself (an expression index over the lower-cased column, or citext on Postgres) and re-create the source key over a BINARY column so sluice's index carry has nothing to refuse. On a non-unique index the collation changes ordering and cost, not which rows are legal, so it is dropped with an INDEX-COLLATION-DROPPED WARN naming the columns. A COLLATE on a column that is part of no index is not yet read, and lands under the target's default collation without a WARN.

An inline or table-level UNIQUE arrives as a constraint under a generated name. SQLite backs each UNIQUE constraint with an auto-index named sqlite_autoindex_<table>_<N>, and every sqlite_-prefixed object name is reserved — a SQLite target refuses to create it (object name reserved for internal use), and used to do so in the index phase, after the whole copy had run. sluice therefore carries such an index as constraint-backed under <table>_<col>[_<col>…]_key, the name Postgres itself generates for the same inline UNIQUE: a Postgres target re-emits ADD CONSTRAINT, MySQL and SQLite a unique index, and uniqueness is never relaxed on any of them. A source CREATE UNIQUE INDEX keeps its own name and is not constraint-backed. A SQLite target also refuses any sqlite_-prefixed index name at preflight — before any data moves — rather than after the copy; that is the shape a backup manifest written by an older binary can still carry.
