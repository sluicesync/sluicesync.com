...
sluice does not leave that implicit. Every CDC open (which is every warm resume, not just a cold start) grades what is actually installed and says so. Four signals arrived across v0.134.1–v0.137.0, a fifth in v0.145.0, and the same single re-run clears all of them:
...
CAPTURE-FUNCTION-PUBLIC-EXECUTE warning · v0.145.0 takes sluice's capture functions off PostgreSQL's default PUBLIC EXECUTE grant. They are SECURITY DEFINER, so with that grant in place any source role able to create a table and a trigger could attach one to a table of its own — named after a synced table — and have every row it wrote applied to the target's real table. An install created before v0.145.0 keeps the grant until setup re-runs. This one had no signal at all until v0.145.0 shipped the warning: the capture-shape door grades the function's body, config and security flag, and never read its ACL, so it was a fifth reason to re-run setup that nothing reported. The reader-side scope check drops any captured row outside the stream's schema in the meantime (a CAPTURE-OUT-OF-SCOPE warning), so an un-revoked install is not exposed to the write primitive itself — re-running setup is what closes it. Audit yours with SELECT proname, proacl FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname = current_schema() AND proname LIKE 'sluice_capture%'; a NULL proacl is the PostgreSQL default, meaning PUBLIC still holds EXECUTE. ·
