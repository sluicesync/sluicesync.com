...
MariaDB's information_schema reports column defaults in a different dialect than MySQL 8: string defaults keep their quotes, a defaultless nullable column's default is the literal word NULL, and DEFAULT CURRENT_TIMESTAMP reads as current_timestamp() with an empty extra. A reader written to MySQL conventions would silently corrupt every default; the mariadb flavor reads them correctly (in the run, DEFAULT CURRENT_TIMESTAMP, DEFAULT 'unnamed', and defaultless-nullable columns all reproduced on the target). That is the cold-start read: under a continuous sync, a forwarded ADD COLUMN did not get the same translation until v0.155.1, and on v0.99.271–v0.155.0 it landed a wrong DEFAULT on the target and filled the target's existing rows with it — see the field note's update for the repair. The catalog also loses some text, which sluice now recovers or refuses: a BINARY/VARBINARY default's high bytes on MariaDB 10.6–11.4 and a character default beyond the Basic Multilingual Plane (v0.156.1), and a character beyond the BMP inside an expression DEFAULT, generated column, CHECK or functional index (v0.156.2) — see type mapping. The same dialect note also covers why SYSTEM VERSIONED tables and SEQUENCEs hide from the BASE TABLE filter: MariaDB reports its defaults in a different dialect.
...
For a zero-downtime cutover (snapshot → CDC → cutover) rather than a one-time copy, a MariaDB source streams its native binlog, but MariaDB's GTIDs are domain-based (e.g. 0-100-38) — a distinct format from MySQL's GTIDs, and sluice parses and resumes off them (since v0.99.271). Two MariaDB CDC realities to plan around, both covered in MariaDB has no BEGIN, and won't tell you if your position survived: a MariaDB transaction opens with a MariadbGTIDEvent and no BEGIN event, and you cannot pre-check whether a stored position is still reachable — @@gtid_binlog_state is unchanged across PURGE BINARY LOGS, so a dead position looks live and the stream throwing error 1236 is the only honest signal. (Whether the position belongs to this server at all — lineage, as distinct from retention — is pre-checked since v0.138.0 through a BINLOG_GTID_POS anchor recorded with the position; see the addendum in that note.) The native uuid/inet columns decode faithfully through this path (v0.99.272).

Set binlog_row_metadata=MINIMAL (or FULL) if you have non-UTF-8 text columns. Since v0.156.3 the binlog decoder converts a latin1, cp1251, sjis, … column's stored bytes to the same UTF-8 the bulk copy carries. Under MINIMAL or FULL each row event records the charset it was written in, so a replay across an ALTER … CHARACTER SET (a warm resume, a lagging stream) is exact. MariaDB's default NO_LOG records none: sluice WARNs CHARSET-HISTORY-UNRECORDED once per such table, refuses a detected replay across a change into a non-UTF-8 charset with SLUICE-E-CDC-SCHEMA-REPLAY-MISMATCH, and leaves a few narrow windows silent. The setting applies only to binlog events written after the change. Details, the remaining windows and the repair for earlier releases: non-UTF-8 character sets.
