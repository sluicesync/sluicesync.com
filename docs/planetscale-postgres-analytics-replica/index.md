...
- Schema forwarding from a Postgres source has an honest per-shape matrix. Column adds, drops, and type changes forward; CREATE INDEX, CHECK, and nullability changes never signal on pgoutput's wire from a Postgres source, so they cannot forward — see the ground-truth table. For indexes that's mostly a feature (the target's index set is yours to design for analytics), but if you want a source index mirrored, create it on the target yourself. Constraints are different since v0.156.0: a source CHECK, NOT NULL, primary-key / UNIQUE / foreign-key, DEFAULT, identity, RLS or policy change stops the stream with UNFORWARDED-SCHEMA-CHANGE at the next write to that table, and it stays stopped until you apply the change on the replica (or decide it does not belong there) and acknowledge it once with --accept-unforwarded-schema-change — see the recovery. If your source team ships such DDL routinely, plan for the stop.
