...
- Large tables — consider --upfront-indexes. By default sluice defers secondary-index creation until after the rows are copied, which is faster overall: building an index once over a finished table beats maintaining it write-by-write. On a large PlanetScale target that default can bite, because the deferred ADD INDEX is a single statement and PlanetScale enforces a statement-time limit — a big enough table can exceed it, and the index build then fails after the copy has already succeeded. --upfront-indexes creates the secondary indexes before the bulk copy and lets the load maintain them, so there is no large post-copy build to time out. It trades a slower copy for removing that failure mode, and it is worth taking on a multi-hundred-GB table, or any table you have previously seen an index build struggle on. Available on both sync start and migrate. If you would rather keep the default and lift the ceiling instead, --planetscale-raise-query-timeout raises the keyspace timeout for the duration of the run and reverts it afterwards — but that is a keyspace-wide change rolled out by a tablet restart, so it affects anyone else on the keyspace.
...
If any table is large — hundreds of GB, or one you have seen a slow index build on — add --upfront-indexes to that second command. It builds the secondary indexes before the copy instead of after, so a post-copy ADD INDEX cannot run into PlanetScale's statement-time limit once the rows are already in place. See the gotcha above for the trade-off.
