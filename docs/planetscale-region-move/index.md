...
Known rough edge — a sharded target rejects sluice's control tables today. A sharded PlanetScale target currently rejects sluice's internal control tables (sluice_cdc_state, sluice_cdc_schema_history, sluice_shard_consolidation_lease) with VT09001: … does not have a primary vindex, which aborts the sync before any rows copy — Vitess requires every table in a sharded keyspace to carry a vindex. Until sluice registers them automatically, the workaround is to add those three control tables to the target keyspace's vschema with a vindex on their primary keys (e.g. unicode_loose_md5 on stream_id / version_key / target_table_full_name) before sync start. This is an honest known rough edge for the sharded-target case; the merge sub-case (A) and the one-shot backup/restore below avoid it entirely.
