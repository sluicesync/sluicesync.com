...
Continuous sync --where has two MySQL-specific limits — neither applies to Postgres, and neither is ever silent.

- A string filter needs a case-sensitive collation on MySQL. MySQL's default string collation is case-insensitive, so a client-side region = 'EU' comparison could disagree with what the source itself would match — a filtered CDC stream refuses it at start (SLUICE-E-WHERE-CDC-UNSUPPORTED-PREDICATE) rather than risk leaking or dropping a row. Give the filter column a _bin / _cs collation, normalize on the source (a generated lower-cased column) and filter on that, or use the one-shot migrate --where (which evaluates on the source, so the source's own collation applies and there is no such restriction). Postgres's default collation is deterministic, so region = 'EU' works directly, as shown above.

- PlanetScale MySQL / Vitess doesn't support sync --where at all. The Vitess VStream cold-start path can't push a row filter down, so sluice refuses sync --where loudly on a planetscale source at start (never a silently partial stream). Use migrate --where for a PlanetScale MySQL region split — it works on planetscale sources (validated).

So: continuous filtered sync is validated end-to-end on PlanetScale Postgres and any Postgres source (real Postgres — set REPLICA IDENTITY FULL as above); self-hosted MySQL works too, subject to the collation note; PlanetScale MySQL uses migrate --where.

The CDC filter accepts a restricted grammar — region = 'EU' is squarely inside it. Because there's no source-side stream filter, sluice evaluates the predicate itself, and it accepts only comparisons it can reproduce byte-for-byte against the source: a column vs a literal (= != <> < <= > >=), IN, IS [NOT] NULL, combined with AND/OR/NOT. A case-insensitive string match, a function call, or a timezone-aware temporal comparison is refused at sync-start (SLUICE-E-WHERE-CDC-UNSUPPORTED-PREDICATE) — a client-side compare could diverge from the source's own and silently leak or drop a row. A plain region/country equality or IN list is exactly the supported shape. See why the grammar is restricted.
