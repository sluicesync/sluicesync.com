...
- PlanetScale MySQL / Vitess pushes the filter server-side. The predicate becomes a VStream filter rule (select * from t where (…)), so Vitess filters both the cold-start copy and the stream; sluice classifies the row-moves client-side. Validated end-to-end on a real Vitess cluster — move-in → INSERT, move-out → DELETE, no leak. One caveat (v0.99.282): the VStream server-side filter is NO-PAD (it ignores a legacy collation's PAD-SPACE trailing-space semantics), so a string filter on a PAD-SPACE collation such as utf8mb4_general_ci is refused at sync-start here rather than silently dropping trailing-space rows — use a NO-PAD utf8mb4_0900_* collation, filter on a different column, or migrate --where. NO-PAD collations (the MySQL 8.0 default) are unaffected.
