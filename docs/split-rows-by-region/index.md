...
Continuous sync --where works across the whole matrix as of v0.99.282 — the one thing to set is full before-images. The sources that can run one are Postgres and the whole MySQL family — self-hosted MySQL, MariaDB, and PlanetScale MySQL / Vitess. (A filtered continuous sync needs the source's change stream to deliver full row before-images; the sqlite, d1 and trigger-CDC engines cannot, so they refuse at preflight rather than filter approximately. migrate --where is a different, wider set: the one-shot copy pushes the predicate into the source read, which the mariadb, mysql, planetscale, postgres, postgres-trigger and vitess readers implement — the flat-file (csv/tsv/ndjson), SQLite/D1 and mydumper readers migrate but refuse --where at preflight rather than silently copying every row.)
...
- PlanetScale MySQL / Vitess pushes the filter server-side. The predicate becomes a VStream filter rule (select * from t where (…)), so Vitess filters both the cold-start copy and the stream; sluice classifies the row-moves client-side. Validated end-to-end on a real Vitess cluster — move-in → INSERT, move-out → DELETE, no leak. PAD-SPACE collations (v0.99.283): the VStream server-side filter is NO-PAD (it ignores a legacy collation's PAD-SPACE trailing-space semantics), so for a string filter on a PAD-SPACE collation such as utf8mb4_general_ci sluice streams that table unfiltered server-side and filters it client-side with the PAD-faithful comparator — the trailing-space 'EU ' a region = 'EU' filter should keep is kept, on both the copy and the stream. The only trade is more wire traffic for that one table. NO-PAD collations (the MySQL 8.0 default) are reduced at the source as usual. (v0.99.282 refused such a filter; v0.99.283 makes it work.) Sharded keyspace, filtering on the shard's vindex column? That's safe: Vitess refuses an in-place UPDATE of a primary vindex column (VT12001), so a row can't silently change shards under the filter and produce a missed move-out — verified on a real 2-shard cluster. One edge that refuses: a single-precision FLOAT ordering term (amount > 0.1) on a PAD-SPACE-forced table is refused (SLUICE-E-WHERE-CDC-UNSUPPORTED-PREDICATE) — the cold-start copy carries single-precision FLOAT display-rounded, so a boundary compare could drop an in-scope row; use DOUBLE, or filter on a non-FLOAT column.

The one requirement everywhere: each filtered table must deliver full row before-images so the move-in/move-out decision can read the old value — MySQL binlog_row_image=FULL, Postgres REPLICA IDENTITY FULL. A filtered UPDATE/DELETE whose before-image omits a predicate column is refused loudly (SLUICE-E-WHERE-CDC-BEFORE-IMAGE, naming the column), never silently mis-classified. (On sluice < v0.99.282, MySQL/PlanetScale string filters were either refused (< v0.99.278) or carried collation edge-case bugs (v0.99.278–281) — upgrade to ≥ v0.99.282, or use migrate --where.)

The CDC filter accepts a restricted grammar — region = 'EU' is squarely inside it. Because there's no arbitrary source-side stream filter, sluice evaluates the predicate over the decoded row and accepts only comparisons it can reproduce faithfully: a column vs a literal (= != <> < <= > >=), IN, IS [NOT] NULL, combined with AND/OR/NOT. Case- and accent-insensitive string equality is supported (evaluated under the column's collation, v0.99.282+); a function call, a subquery, or a timezone-aware temporal comparison is still refused at sync-start (SLUICE-E-WHERE-CDC-UNSUPPORTED-PREDICATE) — a client-side compare could diverge from the source's own. A plain region/country equality or IN list is exactly the supported shape. See why the grammar is restricted.
