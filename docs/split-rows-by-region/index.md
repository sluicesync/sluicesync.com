...
PlanetScale MySQL / Vitess: use migrate --where, not sync --where. Continuous filtered sync needs the source's CDC reader to hand back the full old row so the move-in/move-out decision can read the old region value. The Postgres logical-replication reader and the self-hosted-MySQL binlog reader both do; the Vitess VStream reader (the planetscale driver) does not, so sluice refuses sync --where on a PlanetScale MySQL source at start — a clear SLUICE-E-WHERE-CDC-BEFORE-IMAGE, never a silent leak. For a PlanetScale MySQL region split, use the one-shot migrate --where (which works on every source, VStream included). Continuous filtered sync is fully supported on PlanetScale Postgres (it's real Postgres — set REPLICA IDENTITY FULL as above) and on self-hosted MySQL.
