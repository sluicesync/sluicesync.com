...
DECIMAL(p,s) / NUMERIC · numeric(p,s) · Carried as a string end-to-end; precision is lossless. A bare Postgres numeric (no p/s) is arbitrary-precision — PG→PG round-trips it bare; PG→MySQL widens to DECIMAL(65,30) (MySQL's max) with a loud widening notice. A value that exceeds it is refused, never rounded: past 35 integer digits MySQL refuses the write itself (Error 1264), and since v0.156.4 a value with more significant fractional digits than the target column's scale — past 30 for DECIMAL(65,30), or past a constrained DECIMAL(p,s)'s scale — refuses with SLUICE-E-VALUE-UNREPRESENTABLE marked DECIMAL-SCALE-EXCEEDED, as does a DECIMAL column's literal DEFAULT past the scale. Trailing zeros past the scale lose nothing and are not refused. To carry every digit, --type-override the column to a text type; otherwise round on the source. See values past the scale for the repair on a MySQL target written by an earlier release. ·
...
### Decimal values past the target scale: DECIMAL-SCALE-EXCEEDED (v0.156.4)

An unconstrained Postgres numeric lands on a MySQL-family target as DECIMAL(65,30), MySQL's widest fixed-point form: 35 integer digits and 30 fractional digits. Past 35 integer digits MySQL refuses the write itself (Error 1264). Past 30 fractional digits it does not: MySQL and MariaDB round the value and report only Note 1265, which even strict sql_mode accepts. The bulk copy (LOAD DATA and the batched INSERT path) was always loud, because it refuses a strict-mode write whose warning list is non-empty; the CDC applier never read that list. So through v0.156.3, sync (log-based or postgres-trigger), chain restore, sync from-backup and the added-column backfill stored such values rounded, at exit 0 — measured on MySQL 8.0.46 and MariaDB 11.4: 0.1234567890123456789012345678901 landed as 0.123456789012345678901234567890, and -0.99999999999999999999999999999999 as -1.000000000000000000000000000000. A DECIMAL column's literal DEFAULT past the scale was stored rounded the same way on CREATE TABLE and on a forwarded ADD COLUMN.

Since v0.156.4 sluice refuses any DECIMAL value, and any DECIMAL column's literal DEFAULT, whose significant fractional digits exceed the target column's scale — before the driver sees it, with SLUICE-E-VALUE-UNREPRESENTABLE marked DECIMAL-SCALE-EXCEEDED, naming the column and the digits lost. It applies to a constrained DECIMAL(p,s) column too, and to a float column retyped to a narrower DECIMAL with --type-override. Trailing zeros past the scale lose nothing and are not refused (1.5000…0 lands as 1.5). To proceed, round the value on the source if the digits are not needed, or --type-override the column to a text type to carry every digit. numeric[] values land in a JSON column as strings and were always exact.

If a MySQL, MariaDB or PlanetScale/Vitess target was written by the CDC applier from a Postgres source or chain before v0.156.4 (sync, chain restore or sync from-backup), compare its numeric-sourced DECIMAL columns against the source and re-copy the affected rows: a value with more fractional digits than the target scale is stored rounded, and upgrading does not find or fix it. A target DEFAULT taken from such a source column holds the rounded value — re-declare it from the source. Remaining, stated: a value for a column the applier's descriptor map does not know at all (a cold cache) is not checked. A Postgres target's numeric(p,s) also rounds over-scale input without notice, reachable only through a narrowing --type-override or a pre-existing narrower target column.
...
DECIMAL-SCALE-EXCEEDED (refusal) · Since v0.156.4: a DECIMAL value — or a DECIMAL column's literal DEFAULT, on CREATE TABLE and a forwarded ADD COLUMN alike — with more significant fractional digits than the MySQL, MariaDB or PlanetScale/Vitess target column's scale, typically a Postgres unconstrained numeric past 30 decimal places landing in DECIMAL(65,30). MySQL and MariaDB would round it and report only Note 1265, which even strict sql_mode accepts; sluice refuses before the driver sees it with SLUICE-E-VALUE-UNREPRESENTABLE, naming the column and the digits lost. Trailing zeros past the scale are not refused. Round on the source if the digits are not needed, or map the column to a text type with --type-override. See values past the scale. ·
