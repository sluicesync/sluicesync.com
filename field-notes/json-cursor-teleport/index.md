...
Correction (v0.156.3): the backup value path was not fully protected either. The envelope covered bytes and big integers, but a text value — a Go string — still went to encoding/json bare, so a text value that was not valid UTF-8 was written into the backup chunk as U+FFFD at exit 0, and backup verify agreed with the chunk. Measured on earlier releases during the v0.156.3 investigation: backup full of a SQLite CAST(x'636166e9' AS TEXT) sealed caf�, and a MySQL latin1 column's change-stream value reached backup incremental as raw bytes and restored into Postgres as caf� for café. Since v0.156.3 both chunk write cores check every string before encoding and refuse with BACKUP-VALUE-NOT-UTF8, and the MySQL-family change streams convert legacy-charset columns to UTF-8 first. The resume cursor, which must round-trip whatever key the reader produced, already carried such a string byte-exact and is unchanged. See Backups.
