...
Addendum (sluice v0.138.0): lineage is now pre-checked; retention still is not. The paragraph above is about retention — whether a position still exists — and it stands. A second question, lineage — whether this is even the server that produced the position — turned out to be answerable up front, and MariaDB GTIDs do not answer it on their own: a foreign instance with a different gtid_domain_id, and a rebuilt instance with the same server_id whose history happens to read the same 0-1-3, both accept the position and stream their own history as its continuation (measured on 11.4). Since v0.138.0 each MariaDB position sluice captures carries an anchor — the binlog (file, offset) it was read at, plus BINLOG_GTID_POS(file, offset), the GTID state at that byte of this server's binlog — and a resume asks the source the same question: the same set means the same lineage; NULL with the file present, or a different set, refuses. A purged anchor file is disambiguated with the oldest retained binlog's own start state (BINLOG_GTID_POS(file, 4)), which only the same lineage can produce. Positions captured before the anchor existed resume with an UNVERIFIED-INSTANCE-IDENTITY warning; one fresh full backup or cold start moves the chain onto the check. Error 1236 remains the only honest retention signal.
