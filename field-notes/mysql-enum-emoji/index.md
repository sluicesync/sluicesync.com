...
Update (v0.156.1): the label is lost in the catalog, not in the server — and CDC was silent. Measured on MySQL 8.0.46 and MariaDB 11.4, the server keeps the label intact in the table it executes against; it is every catalog surface (information_schema, SHOW CREATE TABLE, mysqldump) that writes the character as ?. That matters for CDC, which this note did not cover: the binlog carries an ENUM value as its position and a SET as a bitmask, and sluice named them through the catalog's labels, so a row holding '😀b' was applied to the target as '?b' at exit 0 (VStream did the same mapping inside vttablet). Since v0.156.1 such a row stops the stream with ENUM-LABEL-NOT-RECOVERABLE — unless the binlog source runs binlog_row_metadata=FULL, where each row event carries the server's own labels and sluice uses them. So on a binlog source the label is recoverable after all, from the row events. See the marker's entry.

Related (v0.156.1–v0.156.2): labels were not the only catalog text losing characters. A character column's literal DEFAULT beyond the BMP reads back from information_schema as ? too (fixed v0.156.1), and so does text inside an expression — an expression DEFAULT, a generated column, a CHECK, a functional index. There the "2- and 3-byte characters survive" rule above does not hold: MySQL's information_schema renders each stored byte of an expression's string literal as a separate character, so é read back as Ã© and landed on targets that way through v0.156.1. Since v0.156.2 (Bug 288) sluice decodes each literal by its charset introducer and refuses what it cannot decode faithfully. See type mapping.
